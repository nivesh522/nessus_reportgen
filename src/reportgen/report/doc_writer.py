...
def set_cell_shading(cell, fill_color: str) -> None:
    ...
    shd = parse_xml(r'<w:shd xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main" w:fill="%s"/>' % fill_color)
    tcPr.append(shd)


def set_cell_width(cell, width: int, width_type: str = "dxa") -> None:
    tcPr = cell._element.get_or_add_tcPr()
    tcW = parse_xml(r'<w:tcW xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main" w:w="%d" w:type="%s"/>' % (width, width_type))
    tcPr.append(tcW)


def enable_text_wrap(cell) -> None:
    tcPr = cell._element.get_or_add_tcPr()
    text_wrap = parse_xml(r'<w:noWrap xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main" w:val="0"/>')
...
class WordReportWriter:
    ...
    def _add_host_summary(self, doc: Document, findings: List[Finding]) -> None:
        ...
        doc.add_paragraph(f"In total {len(all_ips)} hosts were identified during the scan that had vulnerabilities in them.")
